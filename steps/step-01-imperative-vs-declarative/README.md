...
- **다음 Step 02와의 예습 고리 (Class 캡슐화로의 진화)**:
  - Step 01에서는 `state = {}`와 `function render()`를 따로따로 노출시켜 썼습니다.
  - 하지만 다음 **Step 02**에서는 Step 00 Level 4에서 연습한 **`class Component` 상자** 안에 `this.state`와 `this.render()`를 예쁘게 포장(캡슐화)하여 진짜 프레임워크 같은 공통 컴포넌트 엔진을 완성하게 됩니다!
