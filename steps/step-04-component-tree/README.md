...
## 🔗 [누적 연결] 이전 Step 03과의 비교 및 발전 과정

- **이전 Step 03에서 한 것**: `Component.js`에 `addEvent()` 헬퍼를 탑재하여 이벤트 위임까지 성공했습니다.
- **겪었던 한계**: 하지만 입력창, 목록, 필터 버튼 코드가 모두 하나의 거대한 `App.js` 파일에 1,000줄로 몰려 있어서 유지보수가 불가능했습니다.
- **이번 Step 04에서의 발전**:
  - 단일 책임 원칙(SRP)에 따라 거대한 `App.js`를 **`ItemInput.js`**, **`ItemList.js`**, **`ItemFilter.js`** 자식 모듈들로 쪼갭니다.
  - 부모 ➔ 자식으로는 `props` 데이터를 넘겨주고, 자식 ➔ 부모로는 콜백 함수를 불러 상태 수정을 요청하는 **단방향 데이터 흐름**을 완성합니다!
