...
## 🔗 [누적 연결] 이전 Step 02와의 비교 및 발전 과정

- **이전 Step 02에서 한 것**: `Component` 추상 클래스를 만들고, `setState()`가 불릴 때마다 `this.$target.innerHTML = this.template()` 으로 화면을 통째로 새로 그렸습니다.
- **겪었던 한계**: 화면을 새로 엎어 그릴 때마다 기존에 바인딩해 둔 클릭 이벤트 핸들러가 계속 소멸되거나 중복 등록되어 **메모리 누수 버그**가 생겼습니다.
- **이번 Step 03에서의 발전**:
  - Step 02의 `Component.js` 클래스 내부에 **`addEvent()` 헬퍼 메서드**를 새로 탑재합니다!
  - 이로써 부모 태그(`$target`)에 이벤트를 1회만 고정 등록해 두고 버블링으로 처리하는 **이벤트 위임**으로 프레임워크 엔진을 업그레이드합니다!
