...
## 🔗 [누적 연결] 이전 Step 01과의 비교 및 발전 과정

- **이전 Step 01에서 한 것**: `let state`, `function render()`, `function setState()` 선언적 파이프라인을 직접 작성해 보았습니다.
- **겪었던 한계**: Counter 컴포넌트, Todo 컴포넌트, Modal 컴포넌트 등 새로운 컴포넌트를 만들 때마다 이 파이프라인 코드를 매번 파일마다 처음부터 다시 타이핑해야 했습니다.
- **이번 Step 02에서의 발전**:
  - Step 01에서 만든 파이프라인을 **공통 ES6 추상 클래스(`Component.js`)**로 캡슐화합니다.
  - 이제 어떤 컴포넌트든 `class Counter extends Component` 로 상속만 받으면 1초 만에 컴포넌트가 완성되는 붕어빵 틀 엔진이 구축됩니다!
